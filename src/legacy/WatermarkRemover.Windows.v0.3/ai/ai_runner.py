...
def create_mask(frame_shape, spec, index, fps):
    h, w = frame_shape[:2]
...
def inpaint_video(video, spec, output_video, method, radius, expand_mask):
    ...
    try:
        while True:
            ...
            mask = create_mask(frame.shape, spec, index, fps)
            ...
            writer.write(frame)
            index += 1
            if index % max(1, total // 100) == 0:
                progress(8 + 80 * min(index, total) / total)
    finally:
        cap.release()
        writer.release()
...
def main():
    ...
    try:
        ...
        total_seconds = max(1.0, float(spec.get("duration", 1.0)))
        output.parent.mkdir(parents=True, exist_ok=True)
        progress(90)
        mux_cmd = [
            args.ffmpeg, "-y", "-hide_banner", "-nostdin",
            "-i", str(painted), "-i", str(video),
            "-map", "0:v:0", "-map", "1:a?",
