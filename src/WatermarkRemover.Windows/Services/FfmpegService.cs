...
public sealed class FfmpegService
{
    ...
    private static RegionGeometry Clamp(RegionGeometry r, int maxW, int maxH)
    {
        ...
        // delogo requires the right/bottom edge to stay strictly inside the
        // frame. Auto-detected boxes can otherwise end exactly at width/height
        // and make FFmpeg fail with "Logo area is outside of the frame".
        var w = Math.Clamp(r.Width, 2, Math.Max(2, maxW - x - 1));
        var h = Math.Clamp(r.Height, 2, Math.Max(2, maxH - y - 1));
