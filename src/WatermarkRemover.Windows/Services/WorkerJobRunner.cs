...
public static class WorkerJobRunner
{
    ...
    public static async Task<int> RunFromCommandLineAsync(string[] args)
    {
        ...
        try
        {
            ...
            await File.WriteAllTextAsync(regionsFile!, JsonSerializer.Serialize(snapshots));
            ...
            {
                WriteStatus(statusFile, "running", 5, "正在启动 ProPainter 强力修复…", null);
                var heavy = new HeavyInpaintService();
                await heavy.RunAsync(
                    input,
                    output,
                    regionsFile!,
                    TimeSpan.FromSeconds(startSeconds),
